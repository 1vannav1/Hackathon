...
class GTU:
    def __init__(self, id, specs, average_load_factor=None):
        ...
        self.average_load_factor = (
            average_load_factor  # Как раз коэф загрузки от 0 до 1
        )
        ...
        self.status = (
            "работает"  # Начальное положение всех ГТУ не в резерве для слежения
        )
...
class GTESModel:
    def __init__(self, gtu_specs, load_factors):
        ...
        self.gtus = [
            GTU(i, gtu_specs, load_factors[i])
            for i in range(
    ...
    def assign_cold_reserve(self):
        ...
        eligible_gtus = [
            gtu
            for gtu in self.gtus
            if gtu.status == "работает" and gtu.average_load_factor == 0
        ]
        if eligible_gtus:
            self.cold_reserve = min(eligible_gtus, key=lambda x: x.total_run_hours)
    ...
    def perform_maintenance(self, gtu, current_time):
        ...
        if (
            gtu.run_hours >= self.gtu_specs["ТО_периодичность"]
            and gtu.status == "работает"
        ):
            ...
            gtu.downtime_hours += (
                to_duration.total_seconds() / 3600
        ...
        # Все аналогично для КР
            ...
            gtu.hours_since_kr >= self.gtu_specs["КР_периодичность"]
            and gtu.status == "работает"
        ):
    ...
    def simulate(self, start_date, end_date):  # Собстна, сама симуляция
        ...
        while current_time <= end_date:  # Расчет электроэнергии и времени работы
            ...
            hot_reserve_used = (
                False  # Ограничение для разового использования горячего резерва
            )
            for gtu in need_maintenance:
                if (
                    not hot_reserve_used
                    and self.hot_reserve
                    and self.hot_reserve.status == "горячий_резерв"
                ):
                    print(
                ...
                # Если горячий использован, вводим холодный
                    ...
                    self.cold_reserve and self.cold_reserve.status == "холодный_резерв"
                ):
                    print(
        ...
        print(f"Итоговые расходы на ТО и КР: {self.total_maintenance_cost:.2f} руб.")
        print(
